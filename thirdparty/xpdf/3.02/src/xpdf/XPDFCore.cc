...
#ifdef HIGHLIGHTED_REGIONS
	if (findSelectedRegion(pg, x, y)) {
	  xorHighlight(selectedRegion, highlightNormal, highlightSelected);
	  if (regionResizeX || regionResizeY) {
...
#ifdef HIGHLIGHTED_REGIONS
    if (selectedRegion && (regionResizeX || regionResizeY)) {
...
#ifdef HIGHLIGHTED_REGIONS
      } else if (core->selectedRegion &&
		 (core->regionResizeX || core->regionResizeY)) {
	if (ok) {
	  core->resizeRegion(pg, x, y);
	}
#endif
