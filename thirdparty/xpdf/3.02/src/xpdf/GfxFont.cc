  ...
  if (fontDict->lookup("FontDescriptor", &obj1)->isDict()) {
	...
	if (obj4.isName("Type1")) {
	  ...
	  if (type == fontType1 || type == fontType1C) {
	    ...
	    error(errSyntaxWarning, -1,
		  "Mismatch between font type and embedded font file");
	    type = isCIDFont() ? fontCIDType0C : fontType1C;
	  }
...
char *GfxFont::readExtFontFile(int *len) {
  ...
  if (!(f = fopen(extFontFile->getCString(), "rb"))) {
    error(errIO, -1,
	  "External font file '{0:t}' vanished", extFontFile);
    return NULL;
  }
  fseek(f, 0, SEEK_END);
  *len = (int)ftell(f);
  ...
  if ((int)fread(buf, 1, *len, f) != *len) {
    error(errIO, -1, "Error reading external font file '{0:t}'", extFontFile);
  }
  fclose(f);
  return buf;
}
  ...
  GfxFont(tagA, idA, nameA)
{
  ...
  // check the embedded font file type
  if (embFontID.num >= 0) {
    obj1.initRef(embFontID.num, embFontID.gen);
    obj1.fetch(xref, &obj2);
    if (obj2.isStream()) {
      obj2.streamReset();
      t = FoFiIdentifier::identifyStream(&readFromStream, obj2.getStream());
      obj2.streamClose();
      switch (t) {
      case fofiIdType1PFA:
      case fofiIdType1PFB:
	...
	break;
      default:
	error(errSyntaxWarning, -1, "Embedded font file may be invalid");
	break;
      }
    }
    obj2.free();
  ...
  GfxFont(tagA, idA, nameA)
{
  ...
  if (!desFontDict->lookup("Subtype", &obj1)) {
    error(errSyntaxError, -1,
	  "Missing Subtype entry in Type 0 descendant font");
    goto err2;
  }
