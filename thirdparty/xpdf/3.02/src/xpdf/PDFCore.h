...
#ifdef HIGHLIGHTED_REGIONS
  GList *regions;		// regions on the this page [PDFRegion]
#endif
};
...
public:
  ...
  PDFRegion *addRegion(int pg, double xMin, double yMin,
		       double xMax, double yMax,
		       SplashColor color, SplashColorPtr selectColor,
		       GBool selectable, char *label);
  void deleteRegion(PDFRegion *region);
  void deleteAllRegions();
  int getNumRegions();
  PDFRegion *getRegion(int idx);
  PDFRegion *getSelectedRegion() { return selectedRegion; }
  void setSelectedRegion(PDFRegion *region);
  void makeRegionVisible(PDFRegion *region);
  int loadHighlightFile(char *fileName, SplashColorPtr color,
			SplashColorPtr selectColor, GBool selectable);
  int loadHighlightFile(int (*getChar)(void *stream), void *stream,
...
protected:
  ...
  int loadHighlightFile(HighlightFile *hf, SplashColorPtr color,
			SplashColorPtr selectColor, GBool selectable);
#ifdef HIGHLIGHTED_REGIONS
  GBool findSelectedRegion(int pg, int x, int y);
  void resizeRegion(int pg, int x, int y);
  void xorHighlight(PDFRegion *region, int from, int to,
		    PDFCoreTile *tile = NULL);
#endif
...
#ifdef HIGHLIGHTED_REGIONS
  GList *regions;		// list of highlighted regions [PDFRegion]
  PDFRegion *selectedRegion;	// currently selected region
  int regionResizeX;		// +1/-1/0 = resizing region's right side /
				//   left side / neither
  int regionResizeY;		// +1/-1/0 = resizing region's bottom side /
				//   top side / neither
#endif
