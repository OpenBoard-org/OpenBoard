...
void PSOutputDev::init(PSOutputFunc outputFuncA, void *outputStreamA,
  ...
  for (i = 0; psFonts[i]; ++i) {
    fontNames->add(new GString(psFonts[i]), 1);
  }
  names = globalParams->getPSFontNames();
...
void PSOutputDev::setupFont(GfxFont *font, Dict *parentResDict) {
  ...
  // check for embedded Type 1C font
	     ...
	     font->getEmbeddedFontID(&fontFileID)) {
    psName = makePSFontName(font, &fontFileID);
  ...
  // if font passthrough is requested, just pass the font name through
  // without substitution
  } else if (globalParams->getPSFontPassthrough()) {
    psName = font->getName()->copy();
  ...
  // do 16-bit font substitution
    ...
    if ((uMap = globalParams->getUnicodeMap(fontParam->encoding))) {
