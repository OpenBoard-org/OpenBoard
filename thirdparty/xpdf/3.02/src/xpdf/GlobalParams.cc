...
WinFontInfo *WinFontInfo::make(GString *nameA, GBool boldA, GBool italicA,
  ...
  fileNameA = NULL;
  regName = nameA->copy();
  if (boldA && strcmp(nameA->getCString() + nameA->getLength() - 5, "Black")) {
    regName->append(" Bold");
  }
  if (italicA) {
...
WinFontInfo *WinFontList::find(GString *font) {
  ...
  if (n > 2 && !strcmp(name->getCString() + n - 2, "MT")) {
    name->del(n - 2, 2);
    n -= 2;
  }

  // look for "Regular"
  if (n > 7 && !strcmp(name->getCString() + n - 7, "Regular")) {
    name->del(n - 7, 7);
    n -= 7;
  }

  // look for "Italic"
  if (n > 6 && !strcmp(name->getCString() + n - 6, "Italic")) {
    name->del(n - 6, 6);
    italic = gTrue;
    n -= 6;
  } else {
  ...
  // remove trailing "PS"
  if (n > 2 && !strcmp(name->getCString() + n - 2, "PS")) {
    name->del(n - 2, 2);
    n -= 2;
  }
  ...
  if (!fi) {
    ...
    for (i = 0; i < fonts->getLength(); ++i) {
      ...
      if (!fi->name->cmp(name) && fi->italic == italic) {
	break;
      }
      fi = NULL;
    }
...
void GlobalParams::parseDisplayFont(GList *tokens, GHash *fontHash,
 ...
 err2:
  delete param;
 err1:
  error(errConfig, -1, "Bad 'display*Font*' config file command ({0:t}:{1:d})",
	fileName, line);
}
...
void GlobalParams::parsePSFont(GList *tokens, GString *fileName, int line) {
  ...
  if (tokens->getLength() != 3) {
    error(errConfig, -1, "Bad 'psFont' config file command ({0:t}:{1:d})",
	  fileName, line);
    return;
  }
...
GList *GlobalParams::getPSFontNames() {
  GList *names;
  GHashIter *iter;
  GString *name;
  PSFontParam *p;
  ...
  names = new GList();
  lockGlobalParams;
  psFonts->startIter(&iter);
  while (psFonts->getNext(&iter, &name, (void **)&p)) {
...
GString *GlobalParams::findFontFile(GString *fontName, char **exts) {
  ...
  for (i = 0; i < fontDirs->getLength(); ++i) {
    ...
    for (ext = exts; *ext; ++ext) {
      ...
      fileName = appendToPath(dir->copy(), fontNameU->getCString());
      delete fontNameU;
#else
