...
#ifdef HIGHLIGHTED_REGIONS
  PDFRegion *region;
  for (i = 0; i < regions->getLength(); ++i) {
    region = (PDFRegion *)regions->get(i);
    if (region->page == pg) {
...
void PDFCore::needTile(PDFCorePage *page, int x, int y) {
  ...
  curPage = NULL;
#ifdef HIGHLIGHTED_REGIONS
  for (i = 0; i < page->regions->getLength(); ++i) {
    xorHighlight((PDFRegion *)page->regions->get(i),
		 highlightNone, highlightNormal, tile);
  }
#endif
...
#ifdef HIGHLIGHTED_REGIONS
  // erase any selected highlight
  if (selectedRegion) {
    xorHighlight(selectedRegion, highlightSelected, highlightNormal);
    selectedRegion = NULL;
  }
#endif
...
PDFRegion *PDFCore::addRegion(int pg, double xMin, double yMin,
			      double xMax, double yMax,
			      SplashColorPtr color, SplashColorPtr selectColor,
			      GBool selectable, char *label) {
  PDFRegion *region;
  PDFCorePage *page;

  setSelection(0, 0, 0, 0, 0);
  region = new PDFRegion();
  region->page = pg;
  region->xMin = xMin < xMax ? xMin : xMax;
  region->xMax = xMin < xMax ? xMax : xMin;
  region->yMin = yMin < yMax ? yMin : yMax;
  region->yMax = yMin < yMax ? yMax : yMin;
  splashColorCopy(region->color, color);
  splashColorCopy(region->selectColor, selectColor);
  region->selectable = selectable;
  if (label) {
    region->label = new GString(label);
  } else {
    region->label = NULL;
  }
  ...
  if ((page = findPage(pg))) {
    page->regions->append(region);
    xorHighlight(region, highlightNone, highlightNormal);
  }
  return region;
}

void PDFCore::deleteRegion(PDFRegion *region) {
  PDFCorePage *page;
  int i;

  setSelection(0, 0, 0, 0, 0);
  if ((page = findPage(region->page))) {
    for (i = 0; i < page->regions->getLength(); ++i) {
      if ((PDFRegion *)page->regions->get(i) == region) {
	page->regions->del(i);
	break;
      }
    }
    xorHighlight(region, highlightNormal, highlightNone);
  }
  for (i = 0; i < regions->getLength(); ++i) {
    if ((PDFRegion *)regions->get(i) == region) {
      delete (PDFRegion *)regions->del(i);
      break;
    }
  }
}

void PDFCore::deleteAllRegions() {
  GList *pageRegions;
  PDFRegion *region;
  PDFCorePage *page;
  int i, j;
  ...
  for (i = 0; i < pages->getLength(); ++i) {
    ...
    page->regions = new GList();
    for (j = 0; j < pageRegions->getLength(); ++j) {
      region = (PDFRegion *)pageRegions->get(j);
      xorHighlight(region, highlightNormal, highlightNone);
    }
    delete pageRegions;
  }
...
int PDFCore::getNumRegions() {
  return regions->getLength();
}

PDFRegion *PDFCore::getRegion(int idx) {
  if (idx < 0 || idx >= regions->getLength()) {
    return NULL;
  }
  return (PDFRegion *)regions->get(idx);
}

void PDFCore::setSelectedRegion(PDFRegion *region) {
  // kill the current selection or selected region
  setSelection(selectPage, selectULX, selectULY, selectULX, selectULY);

  // select the new region
  if ((selectedRegion = region)) {
    xorHighlight(region, highlightNormal, highlightSelected);
  }
}

void PDFCore::makeRegionVisible(PDFRegion *region) {
  PDFCorePage *page;
  int xMin, yMin, xMax, yMax, x, y, py;

  if (!(page = findPage(region->page))) {
    displayPage(region->page, zoom, rotate, gFalse, gTrue);
    page = findPage(region->page);
  }
  cvtUserToDev(region->page, region->xMin, region->yMin, &xMin, &yMin);
  cvtUserToDev(region->page, region->xMax, region->yMax, &xMax, &yMax);
  if (xMin > xMax) {
    x = xMin; xMin = xMax; xMax = x;
  }
  if (continuousMode) {
    xMin += scrollX + page->xDest;
    xMax += scrollX + page->xDest;
  }
  if (yMin > yMax) {
    y = yMin; yMin = yMax; yMax = y;
  }
  x = scrollX;
  y = scrollY;
  if (xMax >= x + drawAreaWidth) {
    x = xMax - drawAreaWidth;
  }
  if (xMin < x) {
    x = xMin;
  }
  py = continuousMode ? pageY[region->page - 1] : 0;
  if (py + yMax >= y + drawAreaHeight) {
    y = py + yMax - drawAreaHeight;
  }
  if (py + yMin < y) {
    y = py + yMin;
  }
...
int PDFCore::loadHighlightFile(char *fileName, SplashColorPtr color,
			       SplashColorPtr selectColor, GBool selectable) {
  return loadHighlightFile(HighlightFile::read(fileName),
			   color, selectColor, selectable);
}

int PDFCore::loadHighlightFile(int (*getChar)(void *stream), void *stream,
			       SplashColorPtr color,
			       SplashColorPtr selectColor, GBool selectable) {
  return loadHighlightFile(HighlightFile::read(getChar, stream, "<stream>"),
			   color, selectColor, selectable);
}

int PDFCore::loadHighlightFile(HighlightFile *hf, SplashColorPtr color,
			       SplashColorPtr selectColor, GBool selectable) {
  HighlightRange *range;
  TextOutputDev *textOut;
  int curPage;
  double xi0, yi0, xi1, yi1, x0, y0, x1, y1;
  int i;

  if (!hf) {
    goto err0;
  }
  if (!hf->unitsAreCharacters()) {
    error(errUnimplemented, -1,
	  "Highlight files using word offsets are not supported");
    goto err1;
  }
  ...
  for (i = 0; i < hf->getLength(); ++i) {
    range = hf->get(i);
    if (range->page >= 1 && range->page <= doc->getNumPages()) {
      if (range->page != curPage) {
	doc->displayPage(textOut, range->page, 72, 72, 0,
			 gFalse, gTrue, gFalse);
	curPage = range->page;
      }
      if (textOut->findCharRange(range->pos, range->length,
				 &xi0, &yi0, &xi1, &yi1)) {
	textOut->cvtDevToUser(xi0, yi0, &x0, &y0);
	textOut->cvtDevToUser(xi1, yi1, &x1, &y1);
	addRegion(range->page, x0, y0, x1, y1, color, selectColor,
		  selectable, NULL);
      } else {
	error(errSyntaxError, -1,
	      "Invalid character offset/length in highlight file");
      }
    } else {
      error(errSyntaxError, -1, "Invalid page number in highlight file");
    }
  }
 ...
 err1:
  delete hf;
 err0:
  return errHighlightFile;
}

GBool PDFCore::findSelectedRegion(int pg, int x, int y) {
  PDFCorePage *page;
  PDFRegion *region;
  ...
  if ((page = findPage(pg))) {
    for (i = 0; i < page->regions->getLength(); ++i) {
      region = (PDFRegion *)page->regions->get(i);
      if (region->selectable) {
	cvtUserToDev(region->page, region->xMin, region->yMin, &x0, &y0);
	cvtUserToDev(region->page, region->xMax, region->yMax, &x1, &y1);
	if (x0 > x1) {
	  t = x0; x0 = x1; x1 = t;
	}
	if (y0 > y1) {
	...
	if (x >= x0 - 3 && x < x0 + 3) {
	  if (y >= y0 - 3 && y < y0 + 3) {
	    ...
	    regionResizeY = -1;
	    goto found;
	  } else if (y >= y1 - 3 && y < y1 + 3) {
	    regionResizeX = -1;
	    regionResizeY = 1;
	    goto found;
	  } else if (y >= y2 - 3 && y < y2 + 3) {
	    regionResizeX = -1;
	    regionResizeY = 0;
	    goto found;
	  }
...
void PDFCore::resizeRegion(int pg, int x, int y) {
  int xMinI, yMinI, xMaxI, yMaxI, t;
  int newXMinI, newYMinI, newXMaxI, newYMaxI;
  int wxMin, wyMin, wxMax, wyMax;
  double xMin, yMin, xMax, yMax;
  int sx, sy, py, deltaX;
  GBool needScroll;
  SplashColor regionColor, handleColor;
  PDFCorePage *page;

  // don't allow regions to span multiple pages
  if (pg != selectedRegion->page) {
    return;
  }

  // convert region bounds to screen coordinates
  cvtUserToDev(pg, selectedRegion->xMin, selectedRegion->yMin, &xMinI, &yMinI);
  cvtUserToDev(pg, selectedRegion->xMax, selectedRegion->yMax, &xMaxI, &yMaxI);
  if (xMinI > xMaxI) {
    t = xMinI; xMinI = xMaxI; xMaxI = t;
  }
  if (yMinI > yMaxI) {
    t = yMinI; yMinI = yMaxI; yMaxI = t;
  }

  // move appropriate edge(s) of region
  newXMinI = xMinI;
  newXMaxI = xMaxI;
  newYMinI = yMinI;
  newYMaxI = yMaxI;
  if (continuousMode) {
    page = findPage(selectedRegion->page);
    deltaX = scrollX + page->xDest;
  } else {
    deltaX = 0;
  }
  sx = scrollX - deltaX;
  sy = scrollY;
  py = continuousMode ? pageY[pg - 1] : 0;
  needScroll = gFalse;
  if (regionResizeX < 0) {
    newXMinI = x < xMaxI ? x : xMaxI;
    if (newXMinI < sx) {
      sx = newXMinI;
      needScroll = gTrue;
    } else if (newXMinI > sx + drawAreaWidth) {
      sx = newXMinI - drawAreaWidth;
      needScroll = gTrue;
    }
  } else if (regionResizeX > 0) {
    newXMaxI = x > xMinI ? x : xMinI;
    if (newXMaxI < sx) {
      sx = newXMaxI;
      needScroll = gTrue;
    } else if (newXMaxI > sx + drawAreaWidth) {
      sx = newXMaxI - drawAreaWidth;
      needScroll = gTrue;
    }
  }
  sx += deltaX;
  if (regionResizeY < 0) {
    newYMinI = y < yMaxI ? y : yMaxI;
    if (py + newYMinI < sy) {
      sy = py + newYMinI;
      needScroll = gTrue;
    } else if (py + newYMinI > sy + drawAreaHeight) {
      sy = py + newYMinI - drawAreaHeight;
      needScroll = gTrue;
    }
  } else if (regionResizeY > 0) {
    newYMaxI = y > yMinI ? y : yMinI;
    if (py + newYMaxI < sy) {
      sy = py + newYMaxI;
      needScroll = gTrue;
    } else if (py + newYMaxI > sy + drawAreaHeight) {
      sy = py + newYMaxI - drawAreaHeight;
      needScroll = gTrue;
    }
  }
  cvtDevToUser(pg, newXMinI, newYMinI, &xMin, &yMin);
  cvtDevToUser(pg, newXMaxI, newYMaxI, &xMax, &yMax);
  selectedRegion->xMin = xMin < xMax ? xMin : xMax;
  selectedRegion->xMax = xMin < xMax ? xMax : xMin;
  selectedRegion->yMin = yMin < yMax ? yMin : yMax;
  selectedRegion->yMax = yMin < yMax ? yMax : yMin;

  // setup for drawing
  splashColorCopy(regionColor, paperColor);
  splashColorXor(regionColor, selectedRegion->selectColor);
  splashColorCopy(handleColor, paperColor);

  // erase old region on off-screen bitmap
  xorRectangle(pg, xMinI - 3, yMinI - 3, xMinI + 3, yMinI + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, xMinI - 3, yMaxI - 3, xMinI + 3, yMaxI + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, xMaxI - 3, yMinI - 3, xMaxI + 3, yMinI + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, xMaxI - 3, yMaxI - 3, xMaxI + 3, yMaxI + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, (xMinI + xMaxI)/2 - 3, yMinI - 3,
	       (xMinI + xMaxI)/2 + 3, yMinI + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, (xMinI + xMaxI)/2 - 3, yMaxI - 3,
	       (xMinI + xMaxI)/2 + 3, yMaxI + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, xMinI - 3, (yMinI + yMaxI)/2 - 3,
	       xMinI + 3, (yMinI + yMaxI)/2 + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, xMaxI - 3, (yMinI + yMaxI)/2 - 3,
	       xMaxI + 3, (yMinI + yMaxI)/2 + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, xMinI, yMinI, xMaxI, yMaxI,
	       new SplashSolidColor(regionColor));

  // draw new region on off-screen bitmap
  xorRectangle(pg, newXMinI, newYMinI, newXMaxI, newYMaxI,
	       new SplashSolidColor(regionColor));
  xorRectangle(pg, newXMinI - 3, newYMinI - 3, newXMinI + 3, newYMinI + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, newXMinI - 3, newYMaxI - 3, newXMinI + 3, newYMaxI + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, newXMaxI - 3, newYMinI - 3, newXMaxI + 3, newYMinI + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, newXMaxI - 3, newYMaxI - 3, newXMaxI + 3, newYMaxI + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, (newXMinI + newXMaxI)/2 - 3, newYMinI - 3,
	       (newXMinI + newXMaxI)/2 + 3, newYMinI + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, (newXMinI + newXMaxI)/2 - 3, newYMaxI - 3,
	       (newXMinI + newXMaxI)/2 + 3, newYMaxI + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, newXMinI - 3, (newYMinI + newYMaxI)/2 - 3,
	       newXMinI + 3, (newYMinI + newYMaxI)/2 + 3,
	       new SplashSolidColor(handleColor));
  xorRectangle(pg, newXMaxI - 3, (newYMinI + newYMaxI)/2 - 3,
	       newXMaxI + 3, (newYMinI + newYMaxI)/2 + 3,
	       new SplashSolidColor(handleColor));

  // redraw
  cvtDevToWindow(pg, (xMinI < newXMinI ? xMinI : newXMinI) - 3,
		 (yMinI < newYMinI ? yMinI : newYMinI) - 3,
		 &wxMin, &wyMin);
  cvtDevToWindow(pg, (xMaxI > newXMaxI ? xMaxI : newXMaxI) + 3,
		 (yMaxI > newYMaxI ? yMaxI : newYMaxI) + 3,
		 &wxMax, &wyMax);
  redrawWindow(wxMin, wyMin, wxMax - wxMin + 1, wyMax - wyMin + 1, gTrue);
...
void PDFCore::xorHighlight(PDFRegion *region, int from, int to,
			   PDFCoreTile *tile) {
  int xMinI, yMinI, xMaxI, yMaxI, t;
  int wxMin, wyMin, wxMax, wyMax;
  SplashColor regionColor, handleColor;
  ...
  cvtUserToDev(region->page, region->xMin, region->yMin, &xMinI, &yMinI);
  cvtUserToDev(region->page, region->xMax, region->yMax, &xMaxI, &yMaxI);
  if (xMinI > xMaxI) {
    t = xMinI; xMinI = xMaxI; xMaxI = t;
  }
  if (yMinI > yMaxI) {
  ...
  case highlightNormal:
    splashColorCopy(regionColor, region->color);
    break;
  case highlightSelected:
    splashColorCopy(regionColor, region->selectColor);
    break;
  }
  switch (to) {
  case highlightNone:
    splashColorXor(regionColor, paperColor);
    break;
  case highlightNormal:
    splashColorXor(regionColor, region->color);
    break;
  case highlightSelected:
    splashColorXor(regionColor, region->selectColor);
    break;
  }
  splashColorCopy(handleColor, paperColor);

  if (from == highlightSelected) {
    xorRectangle(region->page, xMinI - 3, yMinI - 3, xMinI + 3, yMinI + 3,
		 new SplashSolidColor(handleColor), tile);
    xorRectangle(region->page, xMinI - 3, yMaxI - 3, xMinI + 3, yMaxI + 3,
		 new SplashSolidColor(handleColor), tile);
    xorRectangle(region->page, xMaxI - 3, yMinI - 3, xMaxI + 3, yMinI + 3,
		 new SplashSolidColor(handleColor), tile);
    xorRectangle(region->page, xMaxI - 3, yMaxI - 3, xMaxI + 3, yMaxI + 3,
		 new SplashSolidColor(handleColor), tile);
    xorRectangle(region->page, (xMinI + xMaxI)/2 - 3, yMinI - 3,
		 (xMinI + xMaxI)/2 + 3, yMinI + 3,
		 new SplashSolidColor(handleColor), tile);
    xorRectangle(region->page, (xMinI + xMaxI)/2 - 3, yMaxI - 3,
		 (xMinI + xMaxI)/2 + 3, yMaxI + 3,
		 new SplashSolidColor(handleColor), tile);
    xorRectangle(region->page, xMinI - 3, (yMinI + yMaxI)/2 - 3,
		 xMinI + 3, (yMinI + yMaxI)/2 + 3,
		 new SplashSolidColor(handleColor), tile);
    xorRectangle(region->page, xMaxI - 3, (yMinI + yMaxI)/2 - 3,
		 xMaxI + 3, (yMinI + yMaxI)/2 + 3,
		 new SplashSolidColor(handleColor), tile);
  }
 
  xorRectangle(region->page, xMinI, yMinI, xMaxI, yMaxI,
	       new SplashSolidColor(regionColor), tile);

  if (to == highlightSelected) {
    xorRectangle(region->page, xMinI - 3, yMinI - 3, xMinI + 3, yMinI + 3,
		 new SplashSolidColor(handleColor), tile);
    xorRectangle(region->page, xMinI - 3, yMaxI - 3, xMinI + 3, yMaxI + 3,
		 new SplashSolidColor(handleColor), tile);
  ...
  if ((page = findPage(region->page))) {
    ...
    cvtDevToWindow(region->page, xMinI, yMinI, &wxMin, &wyMin);
    cvtDevToWindow(region->page, xMaxI, yMaxI, &wxMax, &wyMax);
    redrawWindow(wxMin, wyMin, wxMax - wxMin + 1, wyMax - wyMin + 1, gFalse);
  }
}
