...
#if 1
char*
copy_string(std::string const& str)
{
...
ArgParser::handleArgFileArguments()
{
    ...
    new_argv.push_back(PointerHolder<char>(true, copy_string(argv[0])));
    for (int i = 1; i < argc; ++i)
    {
        ...
        else
        {
            ...
            new_argv.push_back(
                PointerHolder<char>(true, copy_string(argv[i])));
        }
    }
...
int UBExportDocumentCleanerQPDF::Cleanup(QString const &inputFile, QString const &outputFile, QList<int> const &pagesToKeep, const QString &pdfEmptyFileName)
{
    QPDF pdfInput;
    ...
    pdfInput.processInputSource(isInput, "");
    ...
    pdfBlankPage.processInputSource(isBlankPage, "");
    ...
    for (int i = 1; i <= totalPages; i++)
    {
        ...
        if (pagesToKeep.indexOf(i) == -1)
        {
            ...
            page = QString::number(i).toStdString();
        }
