...
void WBHistoryManager::addHistoryEntry(const QUrl &url)
{
    QUrl cleanUrl(url);
    cleanUrl.setPassword(QString());
    cleanUrl.setHost(cleanUrl.host().toLower());

    if (!historyContains(cleanUrl.toString()))
    {
...
void WBHistoryManager::addHistoryItem(const WBHistoryItem &item)
{
    if (!m_history.empty())
    {
        // check: did we just replace http: by https:?
        QUrl itemUrl = item.url;
        QString itemString = itemUrl.toString(
                    QUrl::RemoveScheme |
                    QUrl::RemoveUserInfo |
                    QUrl::StripTrailingSlash);
        QUrl lastItemUrl = m_history[0].url;
        QString lastItemString = lastItemUrl.toString(
                    QUrl::RemoveScheme |
                    QUrl::RemoveUserInfo |
                    QUrl::StripTrailingSlash);

        if (itemString == lastItemString)
        {
...
void WBHistoryManager::save()
{
    ...
    // The tempfile MUST be on the same device, so we create it in the same directory.
    QTemporaryFile tempFile(directory + "/hist");
...
bool WBHistoryMenu::prePopulated()
{
    if (!m_history)
    {
        m_history = UBApplication::webController->browserWindow()->historyManager();
...
{
    return (parent.isValid() || !sourceModel()) ? 0 : sourceModel()->rowCount(parent);
}
